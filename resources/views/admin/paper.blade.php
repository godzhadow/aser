                ...
                <table class="table table-sm table-bordered table-striped w-auto text-small small mw-100">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-primary" data-bs-toggle="collapse" href="#collapse-{{$p->user->id}}" role="button" aria-expanded="false" aria-controls="collapse-{{$p->user->id}}">
                                    click to see team detail
                                </a>
                                <div class="collapse" id="collapse-{{$p->user->id}}">
                                    <div class="card card-body">
                                        <table class="table table-bordered table-striped w-auto">
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td><p class="bg-primary text-white"><b>{{$p->user->name}} (team leader)</b></p></td>
                                                    <td><p class="bg-primary text-white"><b>{{$p->user->university}}</b></p></td>
                                                    <td><p class="bg-primary text-white"><b>{{$p->user->address}},{{$p->city}}</b></p></td>
                                                    <td><p class="bg-primary text-white"><b>{{$p->user->country}}</b></p></td>
                                                </tr>
                                                @foreach ($p->user->team as $team)
                                                <tr>
                                                    <td>{{$team->name}}</td>
                            ...
                            <td>
                                @if ($p->abstract != NULL)
                                    {{-- <a href="{{ url($p->abstract) }}" target="blank"><img width="150px" src="{{ url($p->abstract) }}"></a>
                                    <a href="{{url($p->abstract)}}">download</a> --}}
                                    <button type="button " class="button-link" data-bs-toggle="modal" data-bs-target="#openPDFModal" data-bs-pdf="{{url($p->abstract)}}">
                                        @php
                            ...
                            <td>
                                @if ($p->fullpaper != null)
                                    <button type="button " class="button-link" data-bs-toggle="modal" data-bs-target="#openPDFModal" data-bs-pdf="{{url($p->fullpaper)}}">
                                        @php
                            ...
                            <td>
                                @if ($p->powerpoint != null)
                                <button type="button " class="button-link" data-bs-toggle="modal" data-bs-target="#openPDFModal" data-bs-pdf="{{url($p->powerpoint)}}">
                                    @php
                            ...
                            <td>
                                @if ($p->payment != NULL)
                                    <a href="{{ url($p->payment) }}" target="blank"><img width="150px" src="{{ url($p->payment) }}"></a>
                                @else
                                    <p>no file uploaded yet</p>
                                @endif
                            </td>
                            ...
                            <td class="text-center">
                                <p class=" fw-bold @if($p->abstract_status === 'accepted')
                                    accepted
                                    @elseif($p->abstract_status ==='revision')
                                    revision
                                    @elseif($p->abstract_status ==='rejected')
                                    rejected
                                    @elseif ($p->abstract_status==='under review')
                                    under-review
                                    @endif">{{$p->abstract_status}}</p>
                            </td>
                            <td class="text-center">
                                <p class=" fw-bold @if($p->paper_status === 'accepted')
                                    accepted
                                    @elseif($p->abstract_status ==='revision')
                                    revision
                                    @elseif($p->paper_status ==='rejected')
                                    rejected
                                    @elseif ($p->paper_status==='under review')
                                    under-review
                                    @endif">{{$p->paper_status}}
                                </p>
                            </td>
                            <td class="'text-center">
                                @foreach($p->user->webinar as $conference)
                                <p>{{$conference->start_date}}</p>
                                @endforeach
                            </td>
                            <td>
                                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#validation" data-bs-id="{{$p->id}}" data-bs-name="{{$p->title}}" data-bs-payment="{{$p->payment_status}}" data-bs-abstract-status="{{$p->abstract_status}}" data-bs-paper-status="{{$p->paper_status}}">Validation</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
